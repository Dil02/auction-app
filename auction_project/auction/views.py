...
"""
@csrf_exempt
def available_items(request: HttpRequest)->JsonResponse:
    API for getting items that are currently available
    today = date.today()
    available = Item.objects.filter(start__lte=date.today())
    print(available, " Queryset ")
    print(available[0], "shit")
